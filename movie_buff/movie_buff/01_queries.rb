...
def harrison_ford
  ...
  Movie.joins(:actors).select(:id, :title).where(actors: {name: 'Harrison Ford'}).where.not(castings: {ord: 1})
...
def biggest_cast
  ...
  Movie.joins(:castings).select(:id, :title).group("movies.id").order('COUNT(castings.id) DESC').limit(3)
end
